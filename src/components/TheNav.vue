		...
		<ul class="menu menu-horizontal w-full bg-primary-content flex justify-around gap-2">
			<li>
				<RouterLink class="lg:tooltip" data-tip="Accueil" :to="hasSpa ? '/spa' : '/'" v-slot="{ isActive }">
					<span :class="{ 'active-link': isActive }"
						class="material-symbols-outlined align-middle">pets</span>
				</RouterLink>
			</li>
			<li v-if="!hasSpa">
				<RouterLink class="lg:tooltip" data-tip="Les likes" to="/favorite" v-slot="{ isActive }">
					<span :class="{ 'active-link': isActive }"
						class="material-symbols-outlined fill align-middle">favorite</span>
				</RouterLink>
			</li>
			<li v-else>
				<RouterLink class="lg:tooltip" data-tip="Ajouter" to="/pets/add" v-slot="{ isActive }">
					<span :class="{ 'active-link': isActive || route.name === 'editPet' }"
						class="material-symbols-outlined fill align-middle">{{ route.name === 'editPet' ? 'edit' :
						'add_circle'}}</span>
				</RouterLink>
			</li>
			<li>
				<RouterLink class="lg:tooltip" data-tip="Messages" to="/chats" v-slot="{ isActive }">
					<span :class="{ 'active-link': isActive || route.name === 'chat' }"
						class="material-symbols-outlined fill align-middle">chat</span>
				</RouterLink>
			</li>
			<li>
